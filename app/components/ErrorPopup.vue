    ...
    <Transition enter-active-class="transition duration-200 ease-out" enter-from-class="opacity-0" enter-to-class="opacity-100" leave-active-class="transition duration-150 ease-in" leave-from-class="opacity-100" leave-to-class="opacity-0">
        <div v-if="open" class="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4 backdrop-blur-sm" @click="closeOnOverlayClick">
            <Transition enter-active-class="transition duration-200 ease-out" enter-from-class="opacity-0 scale-95" enter-to-class="opacity-100 scale-100" leave-active-class="transition duration-150 ease-in" leave-from-class="opacity-100 scale-100" leave-to-class="opacity-0 scale-95">
                <div v-if="open" class="w-full max-w-md rounded-2xl border border-zinc-600/10 bg-black/40 p-5 shadow-[0_20px_80px_rgba(255, 0, 0,0.65)] ring-1 ring-white/5 backdrop-blur-md" @click.stop>
                    <div class="flex items-start justify-between gap-4">
                        <div class="flex items-center gap-4">
                            <div class="flex h-11 w-11 items-center justify-center rounded-xl border border-red-400/20 bg-red-500/10 text-lg font-bold text-red-300 shadow-[0_0_22px_rgba(248,113,113,0.16)]"><span>!</span></div>
                        <div>

                        <p class="text-[10px] font-semibold uppercase tracking-[0.26em] text-red-300">Error</p>
                        <h3 class="mt-1 text-lg font-semibold text-zinc-200">Failed to redeem key</h3>
                    </div>
                </div>
